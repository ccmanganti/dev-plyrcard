...
class LocalCoachDatabaseSchoolService
{
    ...
    protected function normalizeSchool(User $user, string $locationId, array $school, ?string $sourceCachedAt): ?array
    {
        ...
        return [
            ...
            'city' => $this->nullableScalarString($school['city'] ?? null),
            'state' => $this->nullableScalarString($school['state'] ?? null),
            ...
            // Query-builder upsert bypasses Eloquent casts, so JSON must be
            // encoded before it reaches SQLite/MySQL.
            'payload' => json_encode(
                $this->lightPayload($school),
    ...
    protected function nullableScalarString(mixed $value): ?string
    {
    ...
    protected function firstScalarString(array $values): string
    {
        foreach ($values as $value) {
            $resolved = $this->nullableScalarString($value);
            if ($resolved !== null) {
                return $resolved;
            }
        }
