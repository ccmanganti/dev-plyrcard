...
trait InteractsWithCoachDatabase
{
    ...
    protected function inboxConversationCacheIsFresh(int $seconds = 60): bool
    {
...
protected function localEmailTemplateToArray(CoachDatabaseEmailTemplate $template): array
    ...
    public function selectConversation(string $conversationId): void
    {
        ...
        $freshCachedMessages = $this->inboxMessageCacheIsFresh($conversationId);
        $this->isLoadingConversationMessages = ! $hasCachedMessages;
        $this->isRefreshingRemoteData = ! $freshCachedMessages;
        $this->activeUiOperation = ($hasCachedMessages || $freshCachedMessages) ? null : 'Loading messages';
        ...
        if (! $freshCachedMessages) {
            $this->startDeferredUiSync('messages', $conversationId);
        }

        // Marking read in GHL is not required to paint the thread. Keep that request after
        // the Livewire response so it cannot delay the user's click.
    ...
    public function refreshConversationMessagesIfStale(string $conversationId, bool $force = false): void
    {
        ...
        $hasCachedMessages = ! empty($this->messages);
