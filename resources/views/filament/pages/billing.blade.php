    ...
    <div class="pc-billing">
        <div class="pc-grid">
            <section class="pc-card">
                ...
                <p class="pc-muted">Manage your billing profile and secure payment method.</p>
                ...
                @if ($billing->card_last_four)
                    <div class="pc-payment">
                        ...
                        <div>
                            ...
                            <span>
                                {{ $billing->card_expiration ? 'Expires '.$billing->card_expiration : 'Secure payment method on file' }}
                                @if($billing->cardholder_name) · {{ $billing->cardholder_name }} @endif
                            </span>
                        </div>
                    </div>
                @else
                    <p class="pc-muted" style="margin-top:.8rem">A saved payment method has not been synchronized yet.</p>
                @endif

                <div class="pc-actions">
                    @if ($this->paymentMethodUpdateUrl)
                        <a class="pc-btn primary" href="{{ $this->paymentMethodUpdateUrl }}">Change Payment Method</a>
                    @else
                        <span class="pc-muted">Secure payment-method updates are not configured yet.</span>
                    @endif
                </div>
                <p class="pc-muted" style="margin-top:.75rem">PLYRCARD stores only limited card metadata. Full card numbers and security codes are never stored here.</p>
            </section>
        </div>

        <section class="pc-card">
            <div class="pc-title">Billing Information</div>
            <p class="pc-muted">This is the primary billing profile used throughout PLYRCARD, including Locker Room.</p>
            ...
            @else
                <div style="overflow:auto">
                    <table class="pc-table">
                        <thead><tr><th>Date</th><th>Status</th><th>Amount</th><th>Method</th></tr></thead>
                        <tbody>
                            @foreach ($this->latestTransactions as $transaction)
                                <tr>
                                    <td>{{ optional($transaction->paid_at ?: $transaction->ghl_created_at)->format('M j, Y') ?: '—' }}</td>
                                    <td>{{ str($transaction->status ?: 'unknown')->replace('_',' ')->title() }}</td>
                                    <td>{{ strtoupper($transaction->currency ?: 'USD') }} {{ number_format(($transaction->amount_cents ?? 0) / 100, 2) }}</td>
                                    <td>{{ $transaction->card_brand ? strtoupper($transaction->card_brand).' •••• '.($transaction->card_last_four ?: '') : '—' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
