...
<x-filament-panels::page>
    <script data-navigate-once>
        (() => {
            const isRecruitingCenter = () => String(window.location.pathname || '').startsWith('/admin/coach-database');
            const disableNoisyAdminHooks = () => {
                if (!isRecruitingCenter()) return;
                window.__plyrcardRecruitingCenterPage = true;
                window.__plyrcardDisableOnboardingAutoStart = true;
                window.PLYRCARD_ONBOARDING_ENABLED = false;
                window.__PLYRCARD_ONBOARDING_DISABLED_FOR_RECRUITING_CENTER = true;
            ...
            if ('serviceWorker' in navigator && isRecruitingCenter()) {
                const key = 'plyrcard:admin-superpwa-unregistered-at';
                ...
                if (!last || (Date.now() - last) > 86400000) {
                    navigator.serviceWorker.getRegistrations()
                        .then((registrations) => Promise.all(registrations.map((registration) => {
                            ...
                            return script.includes('superpwa-sw') ? registration.unregister() : Promise.resolve(false);
                        })))
...
discoverSelectedIds: [],
        ...
        x-on:rc-fast-section.window="activeSection = String($event.detail?.section || activeSection)"
...
<section class="rc-client-panel-v1033" data-rc-client-section="conversations" x-show="activeSection === 'conversations'" style="{{ ($section === 'conversations') ? '' : 'display:none;' }}">
            ...
            @php
                ...
                $inboxAllWindowLimit = min(
                    $filteredConversationTotal,
                    max(10, (int) ($this->inboxConversationDisplayLimit ?? 10))
                );
...
CSS;
            ...
            <style>
                ...
                .rc-email-direct-body-v11320 :where(p, div, table, tbody, tr, td, span, a, ul, ol, li, h1, h2, h3, h4) {
                    max-width:100% !important;
                    overflow-wrap:anywhere !important;
                    word-break:normal !important;
                }
                .rc-email-direct-body-v11320 table {
                    width:auto !important;
                    max-width:100% !important;
                    border-collapse:collapse;
                }
                .rc-email-direct-body-v11320 img {
                    max-width:100% !important;
                    height:auto !important;
                }
                        ...
                        <div
                            ...
                            x-data="{
                                ...
                                init() {
                                    ...
                                    const bootKey = String(window.location.pathname || '') + ':' + String(this.selectedConversationId || '');
                                    if (this.selectedConversationId
                                        && ! this.conversationHasRenderedMessages()
                                ...
                                clearThreadLoading(id, token) {
                                    if (token && token !== this.requestToken) return;
                                    if (!id || this.selectedLoadingId === id) this.selectedLoadingId = '';
                                    if (!id || document.documentElement.getAttribute('data-rc-thread-autoloading') === id) {
                                ...
                                selectConversation(conversationId) {
                                    const id = String(conversationId || '');
                                    if (! id) return;
                                    this.loadConversation(id, false, false);
                                },
                            }"
                            x-init="init()"
                        >
                            @forelse($inboxConversations as $inboxConversation)
                                ...
                                @endphp
                                <button type="button" class="rc-thread-card-v56" x-show="(conversationStatusFilter === 'all' && @js($loop->index < $inboxAllWindowLimit)) || (conversationStatusFilter === 'unread' && @js($unreadCount > 0)) || (conversationStatusFilter === 'incoming' && @js($isIncomingThread)) || (conversationStatusFilter === 'starred' && @js($isStarredThread))" x-bind:class="{ 'is-selected': selectedConversationId === @js($inboxConversationId), 'is-loading': selectedLoadingId === @js($inboxConversationId) }" data-rc-inbox-conversation-trigger data-rc-conversation-id="{{ $inboxConversationId }}" x-on:click.stop="selectConversation(@js($inboxConversationId))">
                            ...
                            @if($canLoadMoreInboxConversations)
                                <div style="padding:.75rem .95rem">
                                    <button type="button" class="rc-btn" style="width:100%" wire:click="loadMoreInboxConversations" wire:loading.attr="disabled" wire:target="loadMoreInboxConversations">
                                        <span wire:loading.remove wire:target="loadMoreInboxConversations">Load 10 more conversations · {{ $inboxAllWindowLimit }} of {{ $filteredConversationTotal }}</span>
                                        ...
                                        <article wire:key="inbox-message-{{ (string) ($message['id'] ?? $loop->index) }}" class="rc-inbox-message-v56 {{ $isOut ? 'is-out' : '' }}">
                                            ...
                                            <div style="min-width:0">
                                                ...
                                                @php
                                                    ...
                                                    $emailBodyForInline = $emailDocument;
                                                    if (preg_match('/<body\b[^>]*>(.*?)<\/body>/is', $emailDocument, $bodyMatch)) {
                                                        $emailBodyForInline = $bodyMatch[1];
                                                    }
                                                    $emailBodyForInline = preg_replace('/<\s*style\b[^>]*>.*?<\s*\/\s*style\s*>/is', '', $emailBodyForInline) ?? $emailBodyForInline;
                                                    $emailBodyForInline = preg_replace('/<\s*script\b[^>]*>.*?<\s*\/\s*script\s*>/is', '', $emailBodyForInline) ?? $emailBodyForInline;
                                                    $emailBodyForInline = preg_replace('/<\s*script\b[^>]*\/?>/is', '', $emailBodyForInline) ?? $emailBodyForInline;
                                                    $emailBodyForInline = preg_replace('/\s+on[a-z]+\s*=\s*("[^"]*"|\'[^\']*\'|[^\s>]+)/i', '', $emailBodyForInline) ?? $emailBodyForInline;
                                                    $emailBodyForInline = preg_replace('/(href|src)\s*=\s*(["\'])\s*javascript:[^"\']*\2/i', '$1="#"', $emailBodyForInline) ?? $emailBodyForInline;
                                                    $emailBodyForInline = preg_replace('/<\/?(?:html|head|body|meta|title|link)\b[^>]*>/i', '', $emailBodyForInline) ?? $emailBodyForInline;
                                                    if (trim(strip_tags($emailBodyForInline)) === '') {
                                                        $fallbackText = trim(strip_tags(html_entity_decode((string) $messageBody, ENT_QUOTES | ENT_HTML5, 'UTF-8')));
                                                        $emailBodyForInline = $fallbackText !== ''
                                                            ? nl2br(e($fallbackText))
                                            ...
                                            @empty
                                                ...
                                                >
                                                    ...
                                                    <button
                                                        ...
                                                        x-on:click.stop="listsOpen=false; window.__plyrRcActivateSectionClientOnly && window.__plyrRcActivateSectionClientOnly('lists', '/admin/coach-database/lists')"
...
<section class="rc-client-panel-v1033" data-rc-client-section="photos" x-show="activeSection === 'photos'" style="{{ ($section === 'photos') ? '' : 'display:none;' }}">
            ...
            <style id="rc-my-photos-v102">
                ...
                .rc-photos-head-v102 h1{margin:0;font-size:1.55rem;line-height:1.05;font-weight:900;letter-spacing:-.04em;color:var(--rc-text)}
                .rc-photos-head-v102 p{margin:.35rem 0 0;max-width:42rem;color:var(--rc-muted);font-size:.78rem;line-height:1.5}
                .rc-photos-mark-v102{width:2.65rem;height:2.65rem;border-radius:.8rem;background:rgba(255,99,56,.11);color:#ff6338;display:grid;place-items:center;flex:0 0 auto}.rc-photos-mark-v102 svg{width:1.25rem;height:1.25rem}
                .rc-photos-tabs-v102{display:inline-grid;grid-template-columns:1fr 1fr;gap:.3rem;padding:.28rem;border:1px solid var(--rc-border);border-radius:.8rem;background:var(--rc-surface);width:min(31rem,100%)}
                .rc-photos-tab-v102{min-height:2.65rem;border:0;border-radius:.58rem;background:transparent;color:var(--rc-muted);font-size:.74rem;font-weight:800;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:.45rem}.rc-photos-tab-v102.is-active{background:var(--rc-soft);color:var(--rc-text);box-shadow:inset 0 0 0 1px var(--rc-border)}
                .rc-photos-count-v102{display:inline-grid;place-items:center;min-width:1.35rem;height:1.35rem;padding:0 .3rem;border-radius:999px;background:rgba(148,163,184,.13);font-size:.61rem}
                .rc-photos-upload-v102{display:flex;align-items:center;justify-content:space-between;gap:1rem;padding:.8rem .9rem;border:1px solid var(--rc-border);border-radius:.85rem;background:var(--rc-surface)}
                .rc-photos-upload-copy-v102{min-width:0}.rc-photos-upload-copy-v102 strong{display:block;color:var(--rc-text);font-size:.78rem}.rc-photos-upload-copy-v102 span{display:block;margin-top:.2rem;color:var(--rc-muted);font-size:.68rem;line-height:1.4}
                .rc-photos-file-v102{position:relative;flex:0 0 auto}.rc-photos-file-v102 input{position:absolute;inset:0;width:100%;height:100%;opacity:0;cursor:pointer}
                .rc-photos-grid-v102{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.75rem}
                .rc-photo-card-v102{position:relative;min-width:0;aspect-ratio:4/3;overflow:hidden;border:1px solid var(--rc-border);border-radius:.9rem;background:var(--rc-soft);box-shadow:0 8px 20px rgba(15,23,42,.045)}
                .dark .rc-photo-card-v102{box-shadow:none}
                .rc-photo-card-v102 img{display:block;width:100%;height:100%;object-fit:cover;transition:transform .22s ease,filter .22s ease}.rc-photo-card-v102:hover img,.rc-photo-card-v102:focus-within img{transform:scale(1.025);filter:brightness(.82)}
                .rc-photo-overlay-v102{position:absolute;inset:0;display:flex;align-items:flex-end;justify-content:flex-end;gap:.4rem;padding:.65rem;background:linear-gradient(to top,rgba(8,15,28,.62),rgba(8,15,28,0) 58%);opacity:0;pointer-events:none;transition:opacity .18s ease}.rc-photo-card-v102:hover .rc-photo-overlay-v102,.rc-photo-card-v102:focus-within .rc-photo-overlay-v102{opacity:1;pointer-events:auto}
                .rc-photo-action-v102{width:2.15rem;height:2.15rem;border:1px solid rgba(255,255,255,.2);border-radius:.62rem;background:rgba(17,24,39,.76);color:#fff;display:grid;place-items:center;cursor:pointer;text-decoration:none;backdrop-filter:blur(5px);transition:transform .14s ease,background .14s ease}.rc-photo-action-v102:hover{transform:translateY(-1px);background:#111827}.rc-photo-action-v102.is-delete:hover{background:#b42318}.rc-photo-action-v102 svg{width:1rem;height:1rem}
                .rc-photos-empty-v102{grid-column:1/-1;min-height:14rem;border:1px dashed var(--rc-border);border-radius:.9rem;background:var(--rc-surface);display:grid;place-items:center;padding:2rem;text-align:center}.rc-photos-empty-v102 strong{display:block;color:var(--rc-text);font-size:.84rem}.rc-photos-empty-v102 span{display:block;margin-top:.3rem;color:var(--rc-muted);font-size:.7rem}
                @media(max-width:1180px){.rc-photos-grid-v102{grid-template-columns:repeat(3,minmax(0,1fr))}}
                @media(max-width:760px){.rc-photos-grid-v102{grid-template-columns:repeat(2,minmax(0,1fr))}.rc-photos-upload-v102{align-items:flex-start;flex-direction:column}.rc-photos-file-v102,.rc-photos-file-v102 .rc-btn{width:100%}}
            ...
            <div class="rc-photos-v102" x-data="{tab:'player'}">
                <div class="rc-photos-head-v102">
                    <div>
                        <h1>My Photos</h1>
                        <p>Keep your player photos and PLYRCARD-ready images together in one clean gallery.</p>
                    </div>
                ...
                <div x-show="tab==='player'" style="display:grid;gap:1rem">
                    <div class="rc-photos-upload-v102">
                        ...
                        <div style="display:flex;gap:.5rem;flex-wrap:wrap;justify-content:flex-end">
                            <div class="rc-photos-file-v102 rc-btn"><span>Choose Photos</span><input type="file" accept="image/*" multiple wire:model="playerPhotoUploads"></div>
                            @if(! empty($playerPhotoUploads))
                                <button type="button" class="rc-btn rc-btn-primary" wire:click="uploadPlayerPhotos" wire:loading.attr="disabled" wire:target="uploadPlayerPhotos,playerPhotoUploads"><span wire:loading.remove wire:target="uploadPlayerPhotos">Upload</span><span wire:loading.flex wire:target="uploadPlayerPhotos" class="rc-loading-inline"><span class="rc-spinner-mini"></span> Uploading</span></button>
                            @endif
                        </div>
                    </div>
                    <div class="rc-photos-grid-v102">
                        @forelse($playerGallery as $photo)
                            @php $photoIndex = (int) ($photo['index'] ?? $loop->index); @endphp
                            <article class="rc-photo-card-v102" wire:key="player-photo-{{ md5((string) ($photo['path'] ?? $photoIndex)) }}">
                                <img src="{{ $photo['url'] ?? '' }}" alt="Player photo" loading="lazy">
                                <div class="rc-photo-overlay-v102">
                                    <a class="rc-photo-action-v102" href="{{ $photo['url'] ?? '#' }}" target="_blank" rel="noopener" download aria-label="Download photo" title="Download"><svg viewBox="0 0 24 24" fill="none"><path d="M12 3v12m0 0 4-4m-4 4-4-4M5 19h14" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></a>
                                    <button type="button" class="rc-photo-action-v102 is-delete" wire:click="deleteMediaPhoto('player','player',{{ $photoIndex }})" wire:confirm="Delete this photo?" aria-label="Delete photo" title="Delete"><svg viewBox="0 0 24 24" fill="none"><path d="M4 7h16M9 7V4h6v3m-8 0 1 13h8l1-13M10 11v5m4-5v5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></button>
                                </div>
                            </article>
                        @empty
                            <div class="rc-photos-empty-v102"><div><strong>No player photos yet.</strong><span>Choose photos above to start your gallery.</span></div></div>
                        @endforelse
                    </div>
                </div>
                ...
                <div x-show="tab==='plyrcard'" x-cloak style="display:grid;gap:1rem">
                    @if($canManagePlyrcardGallery)
                        <div class="rc-photos-upload-v102">
                            <div class="rc-photos-upload-copy-v102"><strong>Additional Photos</strong><span>Add more images to the PLYRCARD gallery.</span></div>
                            <div style="display:flex;gap:.5rem;flex-wrap:wrap;justify-content:flex-end">
                                <div class="rc-photos-file-v102 rc-btn"><span>Choose Photos</span><input type="file" accept="image/*" multiple wire:model="plyrcardPhotoUploads"></div>
                                @if(! empty($plyrcardPhotoUploads))
                                    <button type="button" class="rc-btn rc-btn-primary" wire:click="uploadPlyrcardPhotos" wire:loading.attr="disabled" wire:target="uploadPlyrcardPhotos,plyrcardPhotoUploads"><span wire:loading.remove wire:target="uploadPlyrcardPhotos">Upload</span><span wire:loading.flex wire:target="uploadPlyrcardPhotos" class="rc-loading-inline"><span class="rc-spinner-mini"></span> Uploading</span></button>
                                @endif
                            </div>
                        </div>
                    @endif
                    <div class="rc-photos-grid-v102">
                        @forelse($plyrcardGallery as $photo)
                            @php
                                $photoIndex = (int) ($photo['index'] ?? 0);
                                $photoSource = (string) ($photo['source'] ?? 'additional');
                                $photoField = (string) ($photo['field'] ?? '');
                            @endphp
                            <article class="rc-photo-card-v102" wire:key="plyrcard-photo-{{ md5((string) (($photo['url'] ?? '') . '|' . $photoSource . '|' . $photoField . '|' . $photoIndex)) }}">
                                <img src="{{ $photo['url'] ?? '' }}" alt="PLYRCARD photo" loading="lazy">
                                <div class="rc-photo-overlay-v102">
                                    <a class="rc-photo-action-v102" href="{{ $photo['url'] ?? '#' }}" target="_blank" rel="noopener" download aria-label="Download photo" title="Download"><svg viewBox="0 0 24 24" fill="none"><path d="M12 3v12m0 0 4-4m-4 4-4-4M5 19h14" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></a>
                                    @if($canManagePlyrcardGallery)
                                        <button type="button" class="rc-photo-action-v102 is-delete" wire:click='deleteMediaPhoto("plyrcard", @js($photoSource), {{ $photoIndex }}, @js($photoField))' wire:confirm="Delete this photo?" aria-label="Delete photo" title="Delete"><svg viewBox="0 0 24 24" fill="none"><path d="M4 7h16M9 7V4h6v3m-8 0 1 13h8l1-13M10 11v5m4-5v5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></button>
                                    @endif
                                </div>
                            </article>
                        @empty
                            <div class="rc-photos-empty-v102"><div><strong>No PLYRCARD photos yet.</strong><span>Photos prepared for your PLYRCARD will appear here.</span></div></div>
                        @endforelse
                    </div>
                </div>
            </div>
...
<section class="rc-client-panel-v1033" data-rc-client-section="campaigns" data-rc-server-hydrated="{{ ($section === 'campaigns') ? '1' : '0' }}" x-show="activeSection === 'campaigns'" style="{{ ($section === 'campaigns') ? '' : 'display:none;' }}">
            ...
            <style>
                ...
                .rc-template-card-actions-v50{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:.5rem;align-items:stretch;margin-top:auto}.rc-template-delete-v52{width:1.75rem;height:1.75rem;border:0;background:transparent;color:var(--rc-muted);display:grid;place-items:center;border-radius:.5rem;cursor:pointer}.rc-template-delete-v52:hover{background:rgba(239,68,68,.08);color:#ef4444}.rc-template-body-v52{color:var(--rc-muted);font-size:.74rem;line-height:1.42;min-height:3.25rem;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden}.rc-template-use-v52,.rc-template-edit-v52{height:2.45rem;border-radius:.68rem;font-size:.81rem;line-height:1;font-weight:740;display:inline-flex!important;align-items:center!important;justify-content:center!important;flex-direction:row!important;gap:.42rem!important;white-space:nowrap;cursor:pointer;box-sizing:border-box}.rc-template-use-v52{border:1px solid #ff6338;background:#ff6338;color:#fff;box-shadow:0 7px 16px rgba(255,99,56,.17);padding:0 1rem}.rc-template-use-v52:hover{background:#f95735;border-color:#f95735;box-shadow:0 9px 20px rgba(255,99,56,.22)}.rc-template-edit-v52{min-width:4.7rem;border:1px solid var(--rc-border);background:var(--rc-surface);color:var(--rc-text);padding:0 .78rem;box-shadow:none}.rc-template-edit-v52:hover{border-color:rgba(255,99,56,.42);background:rgba(255,99,56,.055);color:#ff6338}.rc-template-use-v52>span,.rc-template-edit-v52>span{display:inline-flex;align-items:center!important;justify-content:center!important;flex-direction:row!important;gap:.4rem!important;line-height:1!important;margin:0!important;white-space:nowrap}.rc-template-use-v52 svg,.rc-template-edit-v52 svg{display:block!important;width:.95rem!important;height:.95rem!important;min-width:.95rem!important;min-height:.95rem!important;flex:0 0 .95rem!important;margin:0!important}.rc-template-card-actions-v50 button:disabled{cursor:wait;opacity:.68}
                ...
                .rc-template-editor-shell-v11331{border:1px solid rgba(148,163,184,.22);border-radius:1rem;overflow:hidden;background:var(--rc-surface);box-shadow:inset 0 1px 0 rgba(255,255,255,.4)}.rc-template-editor-toolbar-v11331{display:flex;align-items:center;gap:.38rem;flex-wrap:wrap;padding:.62rem;border-bottom:1px solid rgba(148,163,184,.16);background:rgba(248,250,252,.72)}.dark .rc-template-editor-toolbar-v11331{background:rgba(15,23,42,.38)}.rc-template-editor-toolbar-v11331 .rc-select{height:2.1rem;border-radius:.65rem;color:var(--rc-text);background:var(--rc-surface)}.rc-template-editor-toolbar-v11331 .rc-rich-tool{height:2.1rem;min-width:2.1rem;border-radius:.65rem;background:transparent!important;color:var(--rc-text)!important;border-color:transparent!important;opacity:1!important;cursor:pointer}.rc-template-editor-toolbar-v11331 .rc-rich-tool strong,.rc-template-editor-toolbar-v11331 .rc-rich-tool em,.rc-template-editor-toolbar-v11331 .rc-rich-tool u{color:inherit!important}.rc-template-editor-toolbar-v11331 .rc-rich-tool:hover{background:rgba(255,99,56,.10)!important;border-color:rgba(255,99,56,.20)!important;color:#ff6338!important}.rc-template-editor-v50{min-height:24rem!important;padding:1.1rem 1.15rem!important;font-size:.94rem!important;line-height:1.7!important}.rc-template-editor-footer-v11331{display:flex;align-items:center;justify-content:space-between;gap:.75rem;flex-wrap:wrap;padding:.58rem .75rem;border-top:1px solid rgba(148,163,184,.14);color:var(--rc-muted);font-size:.74rem;background:rgba(248,250,252,.45)}.dark .rc-template-editor-footer-v11331{background:rgba(15,23,42,.28)}
                        ...
                        @forelse($templateRows as $template)
                            ...
                            <article class="rc-template-card-v50" wire:key="template-card-v52-{{ $templateId }}">
                                ...
                                <div class="rc-template-card-actions-v50">
                                    ...
                                    ">
                                        <span x-show="!usingTemplate"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M5 12h14"/><path d="m13 6 6 6-6 6"/></svg><span>Use Template</span></span>
                                    ...
                                    <button class="rc-template-edit-v52" type="button" wire:click="selectTemplate({{ \Illuminate\Support\Js::from($templateId) }})" wire:loading.attr="disabled" wire:target="selectTemplate({{ \Illuminate\Support\Js::from($templateId) }})" data-rc-open="template" data-rc-title="{{ $templateNameDisplay }}" data-rc-copy="Opening the editor now. The latest template content will load inside it." data-rc-template-id="{{ $templateId }}" data-rc-template-body-base64="{{ base64_encode($templateBodyRaw) }}" x-on:click="window.__rcTemplateClientMode = 'edit'; window.__plyrTemplateEditorPendingBodyBase64 = $el.dataset.rcTemplateBodyBase64 || ''; window.__plyrTemplateEditorPendingBodyKey = 'card-' + ($el.dataset.rcTemplateId || Date.now()); window.__plyrTemplateEditorFullBodyBase64 = window.__plyrTemplateEditorPendingBodyBase64; window.__plyrTemplateEditorFullBodyKey = window.__plyrTemplateEditorPendingBodyKey;">
                                        <span wire:loading.remove wire:target="selectTemplate({{ \Illuminate\Support\Js::from($templateId) }})"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg><span>Edit</span></span>
                                ...
                                <div class="rc-template-builder-body-v11331">
                                    ...
                                    <div class="rc-template-editor-shell-v11331">
                                        <div class="rc-template-editor-toolbar-v11331" role="toolbar" aria-label="Template editor toolbar">
                                            ...
                                            <button class="rc-rich-tool" type="button" title="Bulleted list" aria-label="Bulleted list" x-on:click="command('insertUnorderedList')">☷</button>
                                            <button class="rc-rich-tool" type="button" title="Numbered list" aria-label="Numbered list" x-on:click="command('insertOrderedList')">☑</button>
                                            <button class="rc-rich-tool" type="button" title="Insert link" aria-label="Insert link" x-on:click="addLink()">🔗</button>
                                            <button class="rc-rich-tool" type="button" title="Insert image" aria-label="Insert image" x-on:click="openImageUpload()">▧</button>
                                        </div>
                                        <input x-ref="imageUpload" type="file" accept="image/*" multiple style="display:none" x-on:change="uploadInlineImages($event)">
...
(() => {
    ...
    const switchSection = (section, href = null, replace = false) => {
        ...
        // The URL, sidebar highlight, and Alpine section state are already updated.
        pendingSection = null;
...
<style id="rc-inbox-list-dropdown-v1075">
    ...
    .rc-profile-action-wrap-v57.rc-school-list-dropdown-v72 > .rc-profile-list-menu-v57.rc-school-list-menu-v72 {
        ...
        max-width: min(13.25rem, calc(100vw - 1.5rem)) !important;
