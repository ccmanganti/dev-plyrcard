@php
    // Never JSON-encode complete school rows here. Some rows contain coach arrays,
    // raw GHL payloads, tags, and tracking metadata; duplicating all of that into
    // Alpine can exhaust PHP memory. Only send fields rendered by this partial.
    $gridSchools = collect($schools ?? [])
        ...
        ->map(function (array $school): array {
            ...
            return [
                ...
                'head_coach_title' => (string) ($school['head_coach_title'] ?? $headCoach['title'] ?? $headCoach['position'] ?? ''),
