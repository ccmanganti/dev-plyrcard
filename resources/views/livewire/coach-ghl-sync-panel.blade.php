...
<div class="ghl-sync-panel" wire:poll.2s.visible="pollStatus" x-data="{ pressed: null }">
    <div class="ghl-sync-bar">
        <div class="ghl-sync-title">
            ...
            <div>
                ...
                <span class="ghl-scope-pill">Sport + gender scoped</span>
                @if($run)
    ...
    @if($run)
        <section class="ghl-sync-status {{ $running ? 'is-running' : '' }}">
            ...
            @if($workerDelayed)
                <div class="ghl-alert">Automatic processing is waiting for the next browser update.</div>
            @endif

            @if(!empty($this->errorSummary))
                <details class="ghl-errors" {{ $run->status === 'paused' ? 'open' : '' }}>
                    <summary>
                        <span>Backend errors</span>
                        <small>Account + GHL subaccount details</small>
                    </summary>

                    <div class="ghl-error-list">
                        @foreach($this->errorSummary as $error)
                            <article class="ghl-error-item">
                                <div class="ghl-error-heading">
                                    <strong>{{ number_format($error['affected']) }} affected</strong>
                                    @if($error['representative_missing'])
                                        <span class="ghl-error-badge is-danger">Credential account missing</span>
                                    @elseif($error['coach_missing'])
                                        <span class="ghl-error-badge is-warning">Local coach missing</span>
                                    @else
                                        <span class="ghl-error-badge">Sync failure</span>
                                    @endif
                                </div>

                                <code>{{ $error['message'] }}</code>

                                <dl class="ghl-error-meta">
                                    <div>
                                        <dt>GHL subaccount</dt>
                                        <dd>{{ $error['location_id'] !== '' ? $error['location_id'] : 'Not recorded' }}</dd>
                                    </div>
                                    <div>
                                        <dt>Credential account</dt>
                                        <dd>{{ $error['representative'] }}</dd>
                                    </div>
                                    @if(!empty($error['linked_accounts']))
                                        <div>
                                            <dt>Accounts sharing this credential</dt>
                                            <dd>{{ implode(' · ', $error['linked_accounts']) }}</dd>
                                        </div>
                                    @endif
                                    @if($error['example_coach'] !== '' || $error['example_school'] !== '')
                                        <div>
                                            <dt>Example affected record</dt>
                                            <dd>
                                                {{ $error['example_coach'] !== '' ? $error['example_coach'] : 'Coach record unavailable' }}
                                                @if($error['example_school'] !== '')
                                                    · {{ $error['example_school'] }}
                                                @endif
                                            </dd>
                                        </div>
                                    @endif
    ...
    <style>
        ...
        .ghl-sync-bar{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:9px 12px}.ghl-sync-title{display:flex;align-items:center;gap:9px;min-width:0}.ghl-sync-title>div{display:flex;align-items:baseline;gap:7px;min-width:0;flex-wrap:wrap}.ghl-sync-eyebrow{font-size:9px;font-weight:900;letter-spacing:.13em;text-transform:uppercase;color:#ff6338}.ghl-sync-title strong{font-size:13px;color:#111827}.dark .ghl-sync-title strong{color:#f8fafc}.ghl-sync-title small{font-size:10px;color:#64748b}.ghl-scope-pill{display:inline-flex;align-items:center;min-height:19px;padding:1px 6px;border-radius:999px;background:#fff1ec;color:#c2410c;font-size:8px;font-weight:850;letter-spacing:.03em}.dark .ghl-scope-pill{background:rgba(255,99,56,.12);color:#fdba74}.ghl-sync-dot{width:8px;height:8px;flex:0 0 auto;border-radius:999px;background:#94a3b8}.ghl-sync-dot.is-live{background:#ff6338;box-shadow:0 0 0 4px rgba(255,99,56,.12);animation:ghlPulse 1.5s ease-in-out infinite}
        ...
        .ghl-stats{display:flex;align-items:center;gap:7px 13px;flex-wrap:wrap;margin-top:8px;font-size:10px;color:#64748b}.ghl-stats span{white-space:nowrap}.ghl-stats b{font-size:12px;color:#111827}.dark .ghl-stats b{color:#f8fafc}.ghl-stats .is-error,.ghl-stats .is-error b{color:#dc2626}.ghl-divider{width:1px;height:17px;background:#e5e7eb}.ghl-current{display:flex;align-items:center;gap:7px;min-width:0;margin-top:8px;padding:6px 8px;border-radius:7px;background:#fff7ed;color:#9a3412;font-size:10px}.ghl-current span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.ghl-current.is-neutral{background:#f8fafc;color:#64748b}.ghl-alert{margin-top:7px;padding:7px 8px;border:1px solid #fed7aa;border-radius:7px;background:#fff7ed;color:#9a3412;font-size:10px}.ghl-errors{margin-top:7px;padding:0;border:1px solid #fecaca;border-radius:9px;background:#fff;overflow:hidden;font-size:10px}.dark .ghl-errors{background:#111827;border-color:rgba(248,113,113,.28)}.ghl-errors>summary{display:flex;align-items:center;justify-content:space-between;gap:8px;cursor:pointer;padding:8px 10px;font-weight:850;color:#991b1b;background:#fef2f2}.dark .ghl-errors>summary{color:#fca5a5;background:rgba(127,29,29,.18)}.ghl-errors>summary small{font-size:9px;font-weight:650;color:#b91c1c}.dark .ghl-errors>summary small{color:#f87171}.ghl-error-list{display:grid;gap:0}.ghl-error-item{padding:10px;border-top:1px solid #fee2e2}.dark .ghl-error-item{border-top-color:rgba(248,113,113,.16)}.ghl-error-heading{display:flex;align-items:center;gap:7px;flex-wrap:wrap}.ghl-error-heading strong{font-size:11px;color:#111827}.dark .ghl-error-heading strong{color:#f8fafc}.ghl-error-badge{display:inline-flex;align-items:center;min-height:20px;padding:2px 6px;border-radius:999px;background:#f1f5f9;color:#475569;font-size:9px;font-weight:800}.ghl-error-badge.is-danger{background:#fee2e2;color:#b91c1c}.ghl-error-badge.is-warning{background:#ffedd5;color:#c2410c}.dark .ghl-error-badge{background:rgba(148,163,184,.12);color:#cbd5e1}.dark .ghl-error-badge.is-danger{background:rgba(239,68,68,.14);color:#fca5a5}.dark .ghl-error-badge.is-warning{background:rgba(249,115,22,.14);color:#fdba74}.ghl-errors code{display:block;margin-top:6px;padding:6px 7px;border-radius:6px;background:#f8fafc;color:#7f1d1d;white-space:normal;word-break:break-word;font-size:10px}.dark .ghl-errors code{background:rgba(15,23,42,.72);color:#fecaca}.ghl-error-meta{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:6px 12px;margin:8px 0 0}.ghl-error-meta>div{min-width:0}.ghl-error-meta dt{margin:0 0 2px;color:#94a3b8;font-size:8px;font-weight:850;letter-spacing:.07em;text-transform:uppercase}.ghl-error-meta dd{margin:0;color:#334155;font-size:10px;font-weight:650;overflow-wrap:anywhere}.dark .ghl-error-meta dd{color:#cbd5e1}.ghl-ready{padding:8px 10px;border:1px dashed #cbd5e1;border-radius:9px;color:#64748b;font-size:10px}.ghl-spinner{width:12px;height:12px;flex:0 0 auto;border:2px solid currentColor;border-right-color:transparent;border-radius:50%;animation:ghlSpin .65s linear infinite}
        ...
        @media(max-width:760px){.ghl-sync-bar{align-items:flex-start;flex-direction:column}.ghl-sync-actions{width:100%}.ghl-btn{flex:1}.ghl-divider{display:none}.ghl-stats{gap:6px 10px}.ghl-error-meta{grid-template-columns:1fr}.ghl-errors>summary{align-items:flex-start;flex-direction:column}}
    </style>
</div>