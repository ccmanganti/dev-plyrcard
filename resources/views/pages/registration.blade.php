...
.steps{list-style:none;margin-top:30px;flex:1}.steps li{display:flex;gap:13px;align-items:center;min-height:45px;padding:10px 0;position:relative}.steps li::before{content:"";position:absolute;left:12px;top:35px;bottom:-10px;width:1px;background:var(--line)}.steps li:last-child::before{display:none}.dot{width:25px;height:25px;flex:0 0 25px;border-radius:50%;border:1px solid var(--line);background:var(--ink);display:grid;place-items:center;font-family:"JetBrains Mono",monospace;font-size:11px;font-weight:700;line-height:1;color:var(--mute);transition:.25s;position:relative;z-index:1}.steps li.on .dot{border-color:var(--coral);color:var(--coral);box-shadow:0 0 0 4px rgba(255,90,60,.11)}.steps li.done .dot{background:var(--coral);border-color:var(--coral);color:#0C0E11}.steps .lb{font-size:13.5px;font-weight:500;line-height:1.25;color:var(--mute);padding-top:0}.steps li.on .lb,.steps li.done .lb{color:var(--paper)}.steps li.on .lb{font-weight:600}.rail-foot{font-size:11.5px;color:var(--mute);line-height:1.6;border-top:1px solid var(--line-soft);padding-top:16px}.rail-foot a{color:var(--paper);text-decoration:none;border-bottom:1px solid var(--line)}
...
<section class="panel" data-step="4">
  ...
  <div class="nav step-nav" id="paymentStartNav">
    ...
    <button type="button" class="btn pri" id="submitRegistration">Continue to secure payment</button>
  </div>
</section>
...
function collectCache(){
  ...
  return {version:23,plan:PLAN,step:cur,division,pickedPositions:[...pickedPositions],chosenDomain,leagueChoice:$('#league')?.value||'',clubChoice:$('#club')?.value||'',teamChoice:$('#team')?.value||'',fields};
}
...
function validateAthleteDetails(){let e=[];e.push(bad($('#grad'),!$('#grad')?.value));return !e.includes(true)}
...
async function paymentStatus(){
  try{
    ...
    const r=await fetch(url.toString(),{headers:{Accept:'application/json','Cache-Control':'no-cache'},cache:'no-store'});
    if(!r.ok)return null;
    return await r.json();
  }catch(e){return null}
}
function applyPaidStatus(data){
  if(!data?.paid||paymentConfirmed)return false;
  paymentConfirmed=true;
  clearInterval(statusPoll);
  const p=$('#paymentProv');p?.classList.add('paid');
  if($('#paymentProvText'))$('#paymentProvText').textContent='Payment confirmed — plan active';
  if($('#paidDoneTitle'))$('#paidDoneTitle').textContent='You’re in';
  if($('#continueProfile'))$('#continueProfile').style.display='inline-flex';
  if($('#doneMessage'))$('#doneMessage').textContent='Your payment is confirmed. Your paid PLYRCARD plan is now active.';
  const watch=$('#paymentWatch');
  if(watch){watch.classList.add('show');watch.innerHTML='<span aria-hidden="true" style="color:var(--good);font-weight:800">✓</span><span><strong>Payment confirmed</strong>Your plan is active. Taking you to the final step…</span>'}
...
function startPaymentPolling(){
  ...
  clearInterval(statusPoll);
  const check=async()=>{const data=await paymentStatus();applyPaidStatus(data)};
  check();
  statusPoll=setInterval(check,3500);
}
