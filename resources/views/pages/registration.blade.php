...
function leagueMatches(row){const g=normalizeGenderValue(division),sport=$('#sport')?.value||'';const gs=(row.genders||[]).map(normalizeGenderValue);return (!gs.length||gs.includes(g)||gs.includes('coed'))&&(!row.sport||row.sport===sport)}
function renderLeagues(){const el=$('#league');if(!el)return;clearSelect(el,'Choose a league');if(!division||!$('#sport')?.value)return;LEAGUES.filter(leagueMatches).sort((a,b)=>a.name.localeCompare(b.name)).forEach(row=>el.add(new Option(row.name,row.id)));el.disabled=false;clearSelect($('#club'),'Choose a league first');clearSelect($('#team'),'Choose a club first');$('#clubOtherWrap')?.classList.remove('open');const toggle=$('#clubOtherToggle');if(toggle){toggle.disabled=true;toggle.classList.remove('active');toggle.textContent='Club not listed? Add your club'}}
...
function renderClubs(){const el=$('#club');if(!el)return;clearSelect(el,'Choose a club');const league=$('#league')?.value,g=normalizeGenderValue(division),sport=$('#sport')?.value||'';const toggle=$('#clubOtherToggle');if(!league){if(toggle)toggle.disabled=true;return}el.add(new Option('Other / Club not listed','__other__'));const rows=CLUBS.filter(row=>String(row.league_id)===String(league)&&(!row.genders?.length||row.genders.map(normalizeGenderValue).includes(g)||row.genders.map(normalizeGenderValue).includes('coed'))&&(!row.sport||row.sport===sport));const seen=new Set;rows.sort((a,b)=>a.name.localeCompare(b.name)).forEach(row=>{if(!seen.has(String(row.id))){seen.add(String(row.id));el.add(new Option(row.name,row.id))}});el.disabled=false;if(toggle){toggle.disabled=false;toggle.classList.remove('active');toggle.textContent='Club not listed? Add your club'}clearSelect($('#team'),'Choose a club first');$('#clubOtherWrap')?.classList.remove('open')}
