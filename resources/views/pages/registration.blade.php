...
<section class="panel" data-step="4">
  ...
  <div class="nav step-nav" id="paymentStartNav"><button type="button" class="btn gho" data-go="3">Back</button><button type="button" class="btn pri" id="submitRegistration">Continue to card payment</button></div>
</section>
...
let cur=1, division='', pickedPositions=[], chosenDomain='', statusPoll=null;
...
function validateAthlete(){let e=[];const dm=$('#divisionWrap .msg');if(dm)dm.classList.toggle('show',!division);e.push(!division);e.push(bad($('#sport'),!$('#sport')?.value));e.push(bad($('#grad'),!$('#grad')?.value));const pm=$('#positionWrap .msg');if(pm)pm.classList.toggle('show',pickedPositions.length===0);e.push(pickedPositions.length===0);e.push(bad($('#st'),!$('#st')?.value));if(PAID||cur===3){e.push(bad($('#league'),!$('#league')?.value));e.push(bad($('#club'),!$('#club')?.value));if($('#club')?.value==='__other__')e.push(bad($('#clubOther'),!$('#clubOther')?.value.trim()));e.push(bad($('#team'),!$('#team')?.value));if($('#ce')?.value)e.push(bad($('#ce'),!mail($('#ce').value))) }return !e.includes(true)}
...
function clearSelect(el,placeholder){if(!el)return;el.innerHTML='<option value="">'+placeholder+'</option>';el.value='';el.disabled=true}
...
function leagueMatches(row){const g=normalizeGenderValue(division),sport=($('#sport')?.value||'').toLowerCase();const gs=(row.genders||[]).map(normalizeGenderValue).filter(Boolean);const sports=(row.sports||[]).map(v=>String(v||'').toLowerCase()).filter(Boolean);const genderMatch=gs.includes(g)||gs.includes('coed');const sportMatch=!sports.length||sports.includes(sport);return genderMatch&&sportMatch}
function renderLeagues(){const el=$('#league');if(!el)return;clearSelect(el,'Choose a league');clearSelect($('#club'),'Choose a league first');clearSelect($('#team'),'Choose a club first');$('#clubOtherWrap')?.classList.remove('open');const toggle=$('#clubOtherToggle');if(toggle){toggle.disabled=true;toggle.classList.remove('active');toggle.textContent='Club not listed? Add your club'}if(!division||!$('#sport')?.value)return;const rows=LEAGUES.filter(leagueMatches).sort((a,b)=>a.name.localeCompare(b.name));if(!rows.length){el.innerHTML='<option value="">No leagues found for this gender and sport</option>';el.disabled=true;return}rows.forEach(row=>el.add(new Option(row.name,row.id)));el.disabled=false}
function setClubOtherMode(active){const club=$('#club'),wrap=$('#clubOtherWrap'),toggle=$('#clubOtherToggle');if(!club||!wrap)return;if(active){club.value='__other__';wrap.classList.add('open');$('#clubOther')?.focus();if(toggle){toggle.classList.add('active');toggle.textContent='Choose a club from the list'}}else{if(club.value==='__other__')club.value='';wrap.classList.remove('open');if(toggle){toggle.classList.remove('active');toggle.textContent='Club not listed? Add your club'}}renderTeams()}
function renderClubs(){const el=$('#club');if(!el)return;clearSelect(el,'Choose a club');const league=$('#league')?.value,g=normalizeGenderValue(division),sport=($('#sport')?.value||'').toLowerCase();const toggle=$('#clubOtherToggle');clearSelect($('#team'),'Choose a club first');$('#clubOtherWrap')?.classList.remove('open');if(!league){if(toggle)toggle.disabled=true;return}el.add(new Option('Other / Club not listed','__other__'));const rows=CLUBS.filter(row=>{const gs=(row.genders||[]).map(normalizeGenderValue).filter(Boolean);const genderMatch=gs.includes(g)||gs.includes('coed');const sportMatch=!row.sport||String(row.sport).toLowerCase()===sport;return String(row.league_id)===String(league)&&genderMatch&&sportMatch});const seen=new Set;rows.sort((a,b)=>a.name.localeCompare(b.name)).forEach(row=>{if(!seen.has(String(row.id))){seen.add(String(row.id));el.add(new Option(row.name,row.id))}});el.disabled=false;if(toggle){toggle.disabled=false;toggle.classList.remove('active');toggle.textContent='Club not listed? Add your club'}}
function renderTeams(){const el=$('#team');if(!el)return;clearSelect(el,'Choose a team / age group');const club=$('#club')?.value;const other=club==='__other__';$('#clubOtherWrap')?.classList.toggle('open',other);const toggle=$('#clubOtherToggle');if(toggle){toggle.classList.toggle('active',other);toggle.textContent=other?'Choose a club from the list':'Club not listed? Add your club'}if(!club){el.disabled=true;return}AGE_GROUPS.forEach(group=>el.add(new Option(group,group)));el.disabled=false}
$('#clubOtherToggle')?.addEventListener('click',()=>setClubOtherMode($('#club')?.value!=='__other__'));
$('#league')?.addEventListener('change',renderClubs);$('#club')?.addEventListener('change',renderTeams);
$$('#divisionWrap button').forEach(b=>b.addEventListener('click',()=>{$$('#divisionWrap button').forEach(x=>x.setAttribute('aria-pressed','false'));b.setAttribute('aria-pressed','true');division=b.dataset.value;$('#genderHidden').value=division;$('#divisionWrap .msg')?.classList.remove('show');const sport=$('#sport');if(sport){sport.disabled=false;sport.value='';sport.options[0].textContent='Choose your sport'};pickedPositions=[];if($('#pos'))$('#pos').innerHTML='<span class="hint">Choose a sport first.</span>';renderLeagues()}));
if($('#sport'))$('#sport').addEventListener('change',function(){pickedPositions=[];const box=$('#pos');box.innerHTML='';const items=POS[this.value]||{};Object.entries(items).forEach(([key,label])=>{const b=document.createElement('button');b.type='button';b.className='chip';b.textContent=label;b.dataset.value=key;b.setAttribute('aria-pressed','false');b.addEventListener('click',()=>{const on=b.getAttribute('aria-pressed')==='true';if(!on&&pickedPositions.length>=3)return;b.setAttribute('aria-pressed',on?'false':'true');pickedPositions=on?pickedPositions.filter(x=>x!==key):pickedPositions.concat(key);$('#positionWrap .msg')?.classList.remove('show')});box.appendChild(b)});if(!Object.keys(items).length)box.innerHTML='<span class="hint">No positions configured for this sport.</span>';renderLeagues()});
...
if(PAID){
 ...
 const updateAutoLink=()=>{const value=slug((($('#fn')?.value||'')+' '+($('#ln')?.value||'')).trim(),true)||'first-last-name';if($('#preview'))$('#preview').textContent=value};$('#fn')?.addEventListener('input',updateAutoLink);$('#ln')?.addEventListener('input',updateAutoLink);$('#terms')?.addEventListener('change',()=>{});updateAutoLink();
}
...
async function submitRegistration(){if(!validate(maxStep))return;clearAlert();rebuildDynamicInputs();setLoading(true);const fd=new FormData(form);try{const r=await fetch(form.action,{method:'POST',body:fd,headers:{Accept:'application/json','X-Requested-With':'XMLHttpRequest'}});const data=await r.json().catch(()=>({}));if(!r.ok){const errors=data.errors||{};const first=Object.values(errors).flat()[0]||data.message||'Registration could not be completed.';showAlert(first);return}if(PAID){const paymentUrl=data.payment_form_url||data.payment_url;if(paymentUrl){const frame=$('#ghlPaymentFrame'),wrap=$('#ghlPaymentWrap'),ready=$('#paymentReady'),nav=$('#paymentStartNav');frame.src=paymentUrl;wrap.classList.add('show');if(ready)ready.style.display='none';if(nav)nav.style.display='none';window.scrollTo({top:wrap.getBoundingClientRect().top+window.scrollY-24,behavior:'smooth'});startPaymentPolling();return}showAlert(data.message||'The card payment form could not be opened. Please try again.');return}else{$('#sent-to').textContent=$('#em').value;$('#doneMessage').textContent=data.message||'Your account is ready.';goto(successStep)}}catch(e){showAlert('We could not complete registration. Check your connection and try again.')}finally{setLoading(false)}}
