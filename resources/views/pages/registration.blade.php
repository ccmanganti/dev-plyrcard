...
function leagueMatches(row){const g=normalizeGenderValue(division),sport=($('#sport')?.value||'').trim().toLowerCase();const gs=(row.genders||[]).map(normalizeGenderValue).filter(Boolean);const sports=(row.sports||[]).map(v=>String(v||'').trim().toLowerCase()).filter(Boolean);const genderMatch=gs.includes(g)||gs.includes('coed');const sportMatch=sports.includes(sport);return genderMatch&&sportMatch}
